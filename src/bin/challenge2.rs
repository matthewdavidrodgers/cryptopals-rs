...
fn main() {
    let buffer_a = hex::decode(
        &("1c0111001f010100061a024b53535009181c".as_bytes().to_vec())
    )
    .unwrap();
    let buffer_b = hex::decode(
        &("686974207468652062756c6c277320657965".as_bytes().to_vec())
    )
    .unwrap();
